...
ngx_int_t
ngx_http_rds_csv_ret_handler(ngx_http_request_t *r)
{
    ngx_chain_t                     *cl;
    ngx_buf_t                       *b;
    size_t                           len;
    ...
    if (errstr.len) {
        escape = ngx_http_rds_csv_escape_json_str(NULL,
    ...
    if (errstr.len) {
        ...
        if (escape == 0) {
            ...
            b->last = (u_char *) ngx_http_rds_csv_escape_json_str(b->last,
                    errstr.data, errstr.len);
        }
    ...
    *b->last++ = '}';
    ...
    if (b->last != b->end) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                "rds_csv: rds_csv_ret: buffer error");
